...
std::string  alu::mNil = "";
#define debug_out ;
//book::debug() << book::fn::function << "rhs=" << rhs << ", this = " << v.d << " : types: " << lexer_component::type_name(T);
...
alu alu::operator/(const alu& rhs)
{
    ...
    lr_number(rhs)
    {
        ...
        if (vr == 0.0L)
        {
            throw book::exception()[ book::except() << book::type::err  << " alu operation: " << book::code::divbyzero << " =>(" << (*this)() << " / " << rhs() << ")" ];
            return "Inf";
        }
        return {lr / vr};
    }
...
alu& alu::operator*=(const alu& rhs)
{
    ...
    {
        book::debug()
            << ui::color::orange5 << "alu"
            << ui::color::white << "::"
            << ui::color::white << book::fn::endl << ":" << book::fn::endl << "{" << book::fn::endl
            << ui::color::yellow << v.d << ui::color::white << '(' << ui::color::lightseagreen << lexer_component::type_name(T) << ui::color::white << ')'
            << ui::color::cornflowerblue << " *= "
            << ui::color::yellow << rhs.v.d << ui::color::white << '(' << ui::color::lightseagreen << lexer_component::type_name(rhs.T) << ui::color::white << ')' ;

        v.d *= rhs.v.d;
        book::out() << ui::color::white << " = " << ui::color::yellow << v.d  << ui::color::white <<  book::fn::endl << '}' <<  book::fn::endl ;
        return *this;
    }
...
alu alu::factorial() const
{
    ...
    lr_number(*this)
    {
        if(v.d < 0)
        {
            throw book::exception()[ book::except() <<" Invalid Base number :" << ui::color::yellow << (*this)() << ui::color::reset] ;
            return {};
        }
        int fac = 1;
        int N = number<int>();
        if (N >= 10)
        {
            throw book::exception()[ book::except() <<" Base number too high." << ui::color::yellow << (*this)() << ui::color::reset];
            return {};
        }
        for (int j = 1; j <= N; j++)
            fac *= j;
        book::debug() << book::fn::function << ui::color::yellow << fac <<ui::color::white << ":"  ;
        return { fac };
    }
    throw book::exception()[ book::except() <<"cannot execute factorial operator on " << lexer_component::type_name(T)];
    return { false };
