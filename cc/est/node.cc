...
node::node(lus::object* a_parent_scope, lex_token* a_token, alu* _a): lus::object(a_parent_scope, a_token ? std::string(a_token->text()) : "node")
{
    ...
    //book::debug() << book::fn::function << "node::node => token details:[" << (t0 ? t0->Details() : "nullptr") << "]"<< book::fn::endl ;
    ...
    switch (_token_->prim) {
        ...
        case lex::type::Number:
        {
            ...
            if (_token_->sem & lex::type::Const)
            {
                ...
                *_a_ = d;
                book::debug() << book::fn::function << " acc: " << ui::color::yellow << (*_a_)();
            }
        }
        return;
        case lex::type::Hex:
    ...
    _op_fn_ = node::s_operator_table.find(a_token->m)->second;
    //book::debug() << book::fn::function << " acc: '" << ui::color::yellow << (*acc)() << ui::color::white << "'";
}
...
alu node::JSR()
{
    //...
    try {
        //book::debug() << book::fn::function << ui::color::white << attribute() << " Value:" << ui::color::yellow << _a_->number<uint64_t>() << book::fn::endl << _token_->Details(true) ;
        if (_token_->is_operator())
        {
            ...
            if (_op_fn_)
                ...
                book::warning() << book::fn::function << "operator node [" << ui::color::yellow << _token_->text() << ui::color::reset << "] has no implementation (yet?).:\n" << _token_->mark();
        }
        else
        {
            if(_op_fn_)
            {
                book::debug() << book::fn::function << " Non operator call: " <<  _token_->mark();
                return (this->*_op_fn_)();
            }
...
alu node::LeftShift()
{
    ...
    {
        *_a_ = 0.f;
        book::warning() << book::fn::function << _l_->_token_->name << " " << _token_->text() << " " << _r_->_token_->type_name() << " are incompatible" ;
    }

    *_a_ = _l_->_a_->number<uint64_t>() << _r_->_a_->number<uint64_t>();
    book::debug() << ui::color::cornflowerblue << " = " << ui::color::lime << (*_a_)();
    return *_a_;
}
...
alu node::Add()
{
    book::debug() << book::fn::function << ui::color::yellow << **_l_ << " " << ui::color::cornflowerblue << _token_->text() << " " << ui::color::yellow << **_r_ << ":";
    *_a_ = *_l_->_a_ + *_r_->_a_;
    book::debug() << ui::color::cornflowerblue << " = " << ui::color::lime << (*_a_)();
    return *_a_;
}
alu node::Sub()
{
    ...
    if (_token_->sem & lex::type::Sign)
        return Negative();
    book::debug() << book::fn::function << ui::color::lime
                     << ui::color::yellow << _l_->value()() << " " << ui::color::cornflowerblue << attribute() << " " << ui::color::yellow << _r_->value()() << ":";
    *_a_ = *_l_->_a_ - *_r_->_a_;
    book::debug() << ui::color::cornflowerblue << " = " << ui::color::lime << (*_a_)();
...
alu node::Mul()
{
    book::debug() << book::fn::function
        << ui::color::yellow << _l_->attribute()
        << ui::color::cornflowerblue << attribute()
        << ui::color::yellow << _r_->attribute();
    *_a_ = *_l_->_a_ * *_r_->_a_;
    book::out() << ui::color::cornflowerblue << " => " << ui::color::lime << (*_a_)() ;
    return *_a_;
}
alu node::Modulo()
{
...
{
    book::debug() << book::fn::function << ui::color::lime
                     << ui::color::aquamarine3 << _l_->attribute() << ui::color::reset << " "
                     << " " << ui::color::cornflowerblue << attribute() << " "
                     << ui::color::yellow
                     << _r_->value()() << ":";
...
alu node::Division()
{
    ...
    *_a_ = *_l_->_a_ / *_r_->_a_;
    book::debug() << ui::color::cornflowerblue << " => " << ui::color::lime << (*_a_)();
    return *_a_;
    //book::debug() << ui::color::cornflowerblue << " = " << ui::color::lime << (*acc)();
}
...
alu node::Factorial()
{
    ...
    book::debug() << book::fn::function << ui::color::lime << _l_->attribute()
                     << ui::color::yellow << " " << (*_l_->_a_)() << " "
                     << ui::color::cornflowerblue << attribute() << ui::color::white << ":" ;

    //book::debug() << node::trace_connect_postfix_operands(this) ;

    *_a_ = _l_->_a_->factorial();

    book::debug() << ui::color::cornflowerblue << " => " << ui::color::yellow << (*_a_)();
    return *_a_;
}
...
alu node::KI16()
{
    book::debug() << book::fn::function << _r_->_token_->text();
...
{
    book::message() << book::fn::function << " Building the associative/move table...";
    if(!node::move_tbl.empty())
    {
        book::error() << book::fn::function << " The associative/move table is already built.";
        return;
    }
...
node* node::tree_input(node* parent_bloc, lex_token* token, node::maker mk)
{
    for (auto& [lr, fntext] : node::input_tbl)
    {
        ...
        {
            book::debug() << ui::color::yellow
            << _token_->text() << " <- "
            << ui::color::yellow << token->text()
            << ui::color::reset
            << " Input TokenPtr validated: '" << ui::color::yellow << fntext << ui::color::reset << "'";
    ...
    book::info() << ui::color::white << "'" << ui::color::yellow << _token_->text() << ui::color::white << "'" << ui::color::reset
                    << "::tree_input(" << ui::color::yellow << token->text() << ui::color::reset << ") => invalid relational operands at "
...
void node::header(node* input_node, std::source_location&& Loc) const
{
    ...
    << ui::color::yellow << _token_->type_name()
    << ui::color::grey100 << "[" << ui::color::blueviolet << _token_->text() << ui::color::grey100 << "] "
    << ui::color::lightsteelblue << " <== "
    << ui::color::yellow << input_node->_token_->type_name()
    << ui::color::grey100 << "[" << ui::color::blueviolet << input_node->_token_->text() << ui::color::grey100 << "]"
...
node* node::collapse_par_pair(node* a)
{
    ...
    if (_l_->_op_) {
        book::debug()
            << ui::color::yellow << _l_->_op_->attribute() << ui::color::cornflowerblue
            << " <-- "
            << ui::color::yellow << a->attribute();
...
node* node::tree_begin(node* ParentObj, lex_token* a_token, node::maker node_maker)
{
    ...
    book::debug() << book::fn::function;
    book::out() << ui::color::yellow << book::fn::endl << a_token->text();
...
node* node::close_tree()
{
    ...
    if (_token_->m == lex::mnemonic::ClosePar) {
        book::debug() << book::fn::function << "Closing the tree on close parenthese:";
        if (_l_)
        {
            node* x = _l_;
            book::debug() << book::fn::function << "left hand side operand: " << _l_->_token_->details() << ":" << book::fn::endl << _l_->_token_->mark();
...
{
    header(this, std::source_location::current());
    //book::debug() << book::fn::function << "Match tree ins from node node:" << book::fn::endl << _token_->Mark() << book::fn::endl ;
    auto* x = this;
    node* p = x;
    do {
        x = p;
...
node* node::set_to_right_operand(node* in_rhs)
{
    ...
    if (_r_) {
        /*
           this
            ...
            rhs
        */
        book::debug() << book::fn::function << _token_->text() << " -> " << _r_->_token_->text()
                         << ui::color::lime << "::tree_set_right"
                         << ui::color::white << " <- "
                         << ui::color::yellow << in_rhs->_token_->text();
        _r_->_op_ = in_rhs;
        in_rhs->_l_ = _r_;
    }
    _r_ = in_rhs;
    ...
    if (_token_->has_type(lex::type::Binary))
    {
        book::debug() << book::fn::function << node::trace_connect_binary_operands(this);
    }
    return in_rhs;
}
