...
[[maybe_unused]] size_t token_table::set_reference_table()
{
    if(!ref_table.empty())
    {
        book::warning() << book::code::notempty << book::fn::function;
    }
...
lex_token const &token_table::get(std::string_view mnemonic_name)
{
    ...
    throw book::exception() [ book::error() << " Tokens identified by '" << ui::color::yellow << mnemonic_name << ui::color::reset << "' Is not in this table."];
}
...
lex_token const &token_table::get(lex::mnemonic::T M)
{
    ...
    throw book::exception()[ book::error()  << " Tokens identified by Enumerator " << ui::color::yellow << static_cast<int>(M) << ui::color::reset << " Is not in this table."];
}
