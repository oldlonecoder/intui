 ...
 * @return cxy& 
 */
cxy &ui::cxy::operator-=(const cxy &dxy)
{
    x -= dxy.x;
    y -= dxy.y;
    return *this;
...
ui::string2d& string2d::operator<<(cxy xy)
{
    if(!r[xy])
    {
...
bool size::has_minmax() const
{
    return (min_size > cxy(0,0)) && (max_size > cxy(0,0));
}
...
{
    return std::format("{},{} [{},{}; {},{}]", cursor.x, cursor.y, a.x, a.y, b.x, b.y, dwh.w, dwh.h);
}
...
cxy rectangle::top_left()
{
    return a;
}

cxy rectangle::top_right()
{
    return {b.x, a.y};
}

cxy rectangle::bottom_left()
{
    return {a.x, b.y};
}

cxy rectangle::bottom_right()
{
    return b;
}
 ...
 * 
 * @param rhs => rhs.a must be downscale to relative coords on calling 
 * @return rectangle 
 */
