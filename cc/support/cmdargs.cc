...
{
    arguments.clear(); // doh ...
    book::info() << book::fn::function << "arguments switches infos are cleared.";
}
...
book::code line::input(const std::vector<std::string_view>& StrArray)
{
    ...
    for(auto sv : StrArray)
    {
        ...
        book::debug() << ui::color::cyan << "line::input" << ui::color::white << '(' << ui::color::yellow << sv << ui::color::white << ") :";
        ...
        if(next == arguments.end())
        {
                ...
                ((*a)->required > (*a)->count) && ((*a)->required > 0))
            {
                ...
                ++(*a)->count;
                (*a)->enabled = true;
                book::debug() << ui::color::yellow << (*a)->name << ui::color::reset << " arg:" << (*a)->count << " '" << sv << '\'';
            }
            else
            {
                // ... or non-switch arg
                ...
                book::debug() << ui::color::yellow << defaults.name << ui::color::reset << " arg:" << defaults.count << " '" << sv << '\'';
            }
        }
        else
        {
            ...
            {
                book::error() << " argument " << ui::color::yellow << (*a)->name << ui::color::reset << " is missing " << ui::color::red4 << (*a)->required - (*a)->count << ui::color::reset << " arguments / " << ui::color::lime << (*a)->required;
    ...
    if (!arguments.empty())
    {
        for (auto* arg : arguments)
        {
            if (arg->enabled)
            {
                ...
                {
                    arg->enabled = false;
                    throw book::except() << " cmd_switch '" << ui::color::yellow << arg->name
                        << ui::color::reset << " is missing "
                        << ui::color::red4 << arg->required - arg->count
                        << ui::color::reset << " arguments: requires "
                        << ui::color::lime << arg->required;
...
std::string line::usage()
{
    ...
    Unused = Used = "%s%s";
    Unused << ui::color::red1 << glyph::unauth;
    Used   << ui::color::lightgreen2 << glyph::ok;
