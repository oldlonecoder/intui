...
object::~object()
{
    ...
    {
        book::out() << "destroy ['" << std::format("{:^20s}",o->id()) << "'] @" << o;
        delete o; 
    }
    m_children.clear();
}
...
[[maybe_unused]] object::iterator object::get_child_iteratorby_id(const std::string &aid)
{
    ...
    std::cerr << ui::color::render(ui::color::white) << id() << " has no child identified by '"
              << ui::color::render(ui::color::yellow) << aid
              << ui::color::render(ui::color::white) << "'\n";
